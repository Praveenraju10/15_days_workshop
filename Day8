...
Data Catalog Assistants — "What does the customer_churn table contain?"
Pipeline Debugging — "Why did ETL job fail with error 504?" (retrieves from incident logs)
SQL Generation — "Write a query for revenue by region" (retrieves schema docs)
Data Governance Q&A — "What is our PII retention policy?" (retrieves policy docs)
Practice Questions
Beginner
What is hallucination in LLMs? Give one real-world example of harm it could cause.
What does RAG stand for? Explain it in 2 sentences.
What is the role of the vector database in the RAG pipeline?
Intermediate
What is the difference between the Indexing phase and the Querying phase?
Why must you use the same embedding model for both documents AND queries?
Why is temperature=0.1 preferred for LLM calls in RAG systems?
Coding
Modify the retrieval function to also print distance scores.
Update the system prompt to make the LLM always answer in bullet points.
Add subject-based filtering to retrieve only from the "GenAI" subject.
Answers
