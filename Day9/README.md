...
What are the four steps of the ReAct pattern?
Why do we use temperature=0.0 for SQL generation?
What information do we include in the schema injected into the system prompt?
What does PRAGMA table_info(students) return?
Why do we include sample rows in the schema? Give a specific example of why they matter.
If the AI returns zero results, what is the most likely cause?
What does re.sub() do in the execute_sql() function?
